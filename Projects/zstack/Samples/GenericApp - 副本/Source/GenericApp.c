...
void UART_MessageMSGCB(MSGfrom_UART * buf);
...
unsigned char ADCV[]="";
uint16 i = 0;
MSGfrom_UART MSGfromUart;


void GenericApp_start_usrt_test();

static void rxCB(uint8 port,uint8 event);
void atoMSG(char * buf, MSGfrom_UART * temp);
...
uint16 GenericApp_ProcessEvent( uint8 task_id, uint16 events )
{
        ...
        case ZDO_STATE_CHANGE:
              ...
              || (GenericApp_NwkState == DEV_END_DEVICE) )
          {
            ...
            osal_start_timerEx( GenericApp_TaskID,
                                GENERICAPP_SEND_MSG_EVT,//ÊÂ¼þ
                                GENERICAPP_SEND_MSG_TIMEOUT );//Ê±¼ä
          }
          break;

       /* case  SPI_INCOMING_ZAPP_DATA: //DMA UART·½Ê½
  ...
  {
    // Send "the" message
    GenericApp_SendPeriodicMessage();
...
/*********************************************************************
 */
void GenericApp_start_usrt_test(void)
{
...
{
  //HalUARTWrite(0,"I get",5);
  unsigned  char Uartbuf[40];
  unsigned char len;
  len=HalUARTRead(0,Uartbuf,40);
  //HalUARTWrite(0,""+len,1);
  if(len)
  {
    HalUARTWrite(0,Uartbuf,40);
    len=0;
  }
}
